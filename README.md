...
**[Agentic RAG System](https://github.com/tamatar-23/agentic-rag)** — 8-node LangGraph pipeline with conditional routing and a self-reflection loop that auto-retries low-faithful answers. Two-tier query router uses keyword heuristics (zero tokens) with LLM fallback — 100% classification accuracy across 3 route types. Avg faithfulness **0.94** on an adversarial 10-question eval suite.

**[T1ME Nifty50](https://github.com/tamatar-23/t1me-nifty50)** — Transformer-based financial forecasting system that reframes market data as token sequences for zero-shot prediction across full 360-minute trading windows. Frozen attention layers cut compute ~60% vs full fine-tuning. Distributed training via DDP + AMP with live data from Fyers API.
